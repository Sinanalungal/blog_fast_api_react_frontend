...
const DEFAULT_IMAGE =
  "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQmEISboQsDjY02hZFABoG-G0a2sXLMfwtJcBO29_j0wYvnLsq6BlAFvLk7y9fZ-_TBM7o&usqp=CAU";
...
const EditPost = ({ blogId, onCancel, onUpdate }) => {
    ...
    validationSchema: Yup.object({
      ...
      shortDescription: Yup.string()
        ...
        .test(
          ...
          (value) => value.replace(/\s/g, "").length >= 50
        )
      ...
      content: Yup.string()
        ...
        .test(
          ...
          (value) => value.replace(/\s/g, "").length >= 150
        )
  ...
  useEffect(() => {
    const fetchBlogDetails = async () => {
      try {
        ...
        console.log(response.data, "this is for fetch blog");
  ...
  const renderImagePreview = () => {
    if (formik.values.image) {
      ...
      return `${BASE_URL}${formik.values.imageUrl}`;
    }
